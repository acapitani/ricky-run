...
func run( obj, delta ):
	"""
	if ( obj.vel.x > 0 and obj.rotate.scale.x > 0 ) or ( obj.vel.x < 0 and obj.rotate.scale.x < 0 ):
		obj.anim_nxt = "run"
	elif ( obj.vel.x > 0 and obj.rotate.scale.x < 0 ) or ( obj.vel.x < 0 and obj.rotate.scale.x > 0 ):
		obj.anim_nxt = "run"
	"""
	if obj.vel.x!=0:
		obj.anim_nxt = "run"
